...
import { BookOpen, User, LayoutGrid, LogOut } from "lucide-react";
      ...
      <div className={cn(
        ...
        <nav className="flex-1 flex flex-col gap-2">
          {navItems.map((item) => {
            ...
            // Oprava logiky: "Učit se" je aktivní jen na "/" nebo v detailu kurzu "/course/..."
            // "/courses" (množné číslo) se už nechytne.
            ...
            return (
              <Link
                ...
                onClick={onClose}
                className={cn(
                  "flex items-center gap-4 px-4 py-3 rounded-2xl font-black transition-all group",
                  isActive
                    ? "bg-duo-green/10 text-duo-green border-2 border-duo-green/20"
                    : "text-duo-gray-dark hover:bg-duo-gray/10"
                )}
              >
                <item.icon className={cn("size-7 transition-transform group-active:scale-90", isActive ? "text-duo-green" : "text-duo-gray-dark")} />
